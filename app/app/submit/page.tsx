...
export default function SubmitPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const result = await getGenLayerClient().submitContent(content.trim(), type, parseGen(bond))
      if (!result.success) throw new Error(result.error || 'Submission failed')
      const id = typeof result.data === 'string' ? result.data.replace(/"/g, '') : ''
      setSubmissionId(id)
      setMessage(`Submission finalized${result.hash ? `: ${result.hash.slice(0, 12)}...` : ''}`)
      setContent('')
    } catch (error) {
      ...
      <div className="grid gap-5 lg:grid-cols-[1.35fr_0.85fr]">
        <form onSubmit={submit} className="assist-card rounded-[26px] p-6 md:p-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
            ...
            <button disabled={busy || !content.trim()} className="assist-btn-primary h-14 rounded-full px-8 font-bold disabled:opacity-50">{busy ? 'Finalizing...' : 'Lock bond & submit'}</button>
          </div>
